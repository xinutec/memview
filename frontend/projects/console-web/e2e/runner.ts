...
export interface Runner {
  base: string;
  /** Everything the runner has said. It logs a refused push with both lengths
   *  and the revision, which is what tells "the client never sent" apart from
   *  "the runner said no". */
  said: () => string;
  stop: () => void;
}
...
export async function start(port: number, staticDir: string): Promise<Runner> {
  ...
  for (let attempt = 0; attempt < 150; attempt++) {
    ...
    if (seen) {
      ...
      return { base, said: () => log, stop: () => stop(child, root) };
    }
