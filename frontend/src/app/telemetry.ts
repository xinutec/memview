...
export function labelFor(node: EventTarget | null): string | null {
  ...
  // 'warningProblems'; and aria-hidden content is by definition not part
 ...
 * Read together, the two streams make it answerable. The graph view makes the
 * gap unusually wide: turning the camera, changing the cluster grain and
 * walking between memories are all silent as far as the API is concerned.
 *
...
export class Telemetry {
  ...
  private flush(final: boolean): void {
    ...
    // Straight to HttpClient rather than through MemviewApi: that class is the
    // typed read surface for the corpus, and a fire-and-forget write does not
    // belong in it. Empty error handler — a trace that surfaces its own failures
    // interferes with the app it observes.
    this.http.post<void>('/api/telemetry', batch).subscribe({ error: () => {} });
  }
}
