...
interface Place {
  ...
  reads: number;
  writes: number;
  /** Width of the bar, 0..1, against this agent's busiest project. */
  share: number;
}
...
export class AgentsView {
  ...
  private row(a: Agent): AgentRow {
    ...
    const places: Place[] = [...projects]
      ...
      // Writes first, because that is what "works on" means here; reads break
      // the tie among places the agent has only ever consulted.
      .sort((x, y) => y.writes - x.writes || y.reads - x.reads)
    ...
    const busiest = Math.max(1, ...places.map((p) => p.reads + p.writes));
    for (const p of places) p.share = (p.reads + p.writes) / busiest;
