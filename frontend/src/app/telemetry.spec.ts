...
describe('labelFor', () => {
  it('reads the accessible name in preference to the text', () => {
    const el = markup('<button aria-label="Mute this check">volume_off</button>');
    expect(labelFor(el)).toBe('Mute this check');
  });
  ...
  it("strips a Material icon's ligature name out of the label", () => {
    ...
    // without stripping it every icon+label control logs as "warningProblems"
    // and the trace is unreadable.
    const el = markup('<a><mat-icon>warning</mat-icon>Problems</a>');
    expect(labelFor(el)).toBe('Problems');
  });
  ...
  it('finds the control a tap landed inside', () => {
    ...
    const button = markup('<button><span class="label">Overview</span></button>');
    expect(labelFor(button.querySelector('.label'))).toBe('Overview');
  });
  ...
  it('says nothing for a tap that missed every control', () => {
    ...
    expect(labelFor(markup('<p>No problems reported.</p>'))).toBeNull();
  ...
  it('does not disturb the live DOM while reading a label', () => {
    ...
    const el = markup('<button><mat-icon>dashboard</mat-icon>Overview</button>');
    document.body.append(el);
    expect(labelFor(el)).toBe('Overview');
