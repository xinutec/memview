...
// run (the L2 phone-width layout harness — see @xinutec/ui-harness, repo
// ~/Code/ui-harness, + dev-lint/docs/layout-quality-architecture.md). Serves
// dist/memview-web/browser with SPA fallback to index.html and correct content
...
// leave unrouted. Matches messages/life/fleetwatch's e2e/serve.mjs.
import { createServer } from 'node:http';
import { readFile, stat } from 'node:fs/promises';
import { extname, join, normalize } from 'node:path';
import { fileURLToPath } from 'node:url';

const ROOT = join(fileURLToPath(new URL('.', import.meta.url)), '..', 'dist', 'memview-web', 'browser');
...
// backend. Signed-in owner + an empty corpus, so an un-mocked run still renders.
const API = {
