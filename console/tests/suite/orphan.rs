...
//!
//! **Do not fix by guessing which spawn.** Waiting is a poor way to find out:
//! over 50 hours the console ran 17 gist sweeps with 0 failures and 3 deaf
//! captures with 0 probe timeouts, so neither suspect's failing path executed
//! once. Zero zombies in that window says nothing about either shape.
//!
//! So force the condition. Each test spawns a child that outlives a deliberately
//! short timeout, drops the wait as `tokio::time::timeout` does, and asks the
//! process table about THAT pid. The child is short-lived because an unreaped
//! child is only visible as a zombie once it exits.
//!
//! The two shapes differ in one flag:
//!
