...
//!
//! Run by `scripts/enrol.sh`. Exits non-zero if anything failed, and prints the
//! pin only when nothing did, so a pipeline reading its output cannot enrol a
//! phone the check rejected — a key pinned on a bad chain looks exactly like a
