//! Who wrote a memory, when its own frontmatter does not say. A heredoc write
//! skips the stamping path and carries no `originSessionId`, so
//! `lint::passed_for_session` matches it to nobody — but the transcripts record
//! the write, and `memory-stamp` has recovered the author from them since it
//! was built. Moved out of the binary so the linter can ask (memview#1235: a
//! corpus sat uncommittable for four hours because nobody was told).
...
/// Which session wrote each wanted file, from the transcripts. A mention is not
/// a write: the filename must be a `file_path` on a writing tool, or a file the
/// READER says the command wrote. Checked against six memories with known
/// authors, all six right. `wanted` is file NAMES with their extension.
    ...
    for path in transcripts(root) {
        ...
        for line in text.lines() {
            ...
            for item in content {
                ...
                // The EARLIEST write is the creation; `originSessionId` means who wrote it first.
...
/// Does `command` WRITE `name`, rather than just mentioning it? Asked of the
/// reader: a scan for `>` argues with the shell about `tee`, heredocs and
/// redirects glued to the word before.
...
/// The marker that makes filing idempotent: the nightly runs every day, and one
/// stale error would file thirty tasks in a month. `task add`'s own duplicate
/// check warns and files anyway.
...
/// This agent's existing open `[memory-lint]` task. Open only: refreshing a
/// closed one would reopen a finished conversation.
...
/// The earliest write of EVERY memory the transcripts record, including ones
/// since deleted — the creation record needs the other question from
/// [`attribute`]. The shell arm goes through the reader, where the script this
/// replaces tokenised on `>`, `tee` and `mv`.
