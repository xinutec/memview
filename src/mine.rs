...
//!
//! [`reader::watermark::plan`] decides WHICH transcripts a run must read. This module
...
//!     its transcript, so a tail-only read cannot re-derive it. Without this, a
//!     resumed run files every long-running session under its raw uuid, which reads
//!     as "every agent was renamed overnight".
//!   * **`first_seen`** — the earliest sighting of each commit hash. A tail read sees
//!     only later ones, so the dates would creep forward run after run.
//!   * **per-agent `DaysSeen`** — the day sets behind each agent's weights. Only
//!     their union survives, in `memory-days.json`, and a union cannot be taken apart
...
//! mine, silently and with no error. The two files are separate so that one tool
//! writing cannot move the other tool's floor.
//!
...
pub struct Carried {
    ...
    /// toggle them.
    ///
    /// So renames are treated the way commit attribution is: a derivation from raw state,
    /// recomputed each run, never accumulated. Found by the first full-corpus parity run
    /// — no fixture has a rename in it, let alone a cyclic one.
    #[serde(default)]
...
impl Carried {
    ...
    /// mine. The failure is a slow run, not a wrong one — and being fatal instead would
    /// abort the nightly under `set -euo pipefail`.
    ///
    /// The damage is still named on stderr, because a resume file that will not parse is
    /// worth somebody knowing about even when the run recovers.
