//! What the JavaScript reader can make of the history's JavaScript, and what it
//! cannot.
//!
//!     cargo run --release --bin javascript-report -- <corpus.jsonl> [--show <n>] [--why <substring>]
//!
//! The fifth of the family, and deliberately `python-report` with one word
//! changed: the two languages are measured the same way, so a figure from one
//! can be read against a figure from the other without asking whether the
//! denominators mean the same thing.
//!
//! There is no parse-failure figure here, for the reason `python-report` gives:
//! `javascript.pest` accepts punctuation it has no reading for, so a program is
...
//!
//! One figure this has that the Python one does not: **programs that did not
//! run**. `javascript::did_not_run` refuses text whose brackets or quotes never
//! close, which is what a heredoc cut short looks like, and a refusal there
//! deletes real file operations — so it is printed rather than left implicit.
...
fn main() -> anyhow::Result<()> {
    ...
    let Some(path) = args.get(1) else {
        anyhow::bail!("usage: javascript-report <corpus.jsonl> [--show <n>] [--why <substring>]");
    };
    let show = args
        .iter()
        .position(|a| a == "--show")
    ...
    let mut tally = javascript::Tally::default();
    let mut did_not_run: BTreeMap<String, usize> = BTreeMap::new();
    ...
    for line in text.lines() {
        ...
        for op in &found.ops {
            ...
            let program = javascript::read(source);
            if let Some(why) = program.did_not_run {
                *did_not_run.entry(why.to_string()).or_insert(0) += 1;
            }
    ...
    println!("javascript programs   {}", tally.programs);
    println!(
    ...
    if !did_not_run.is_empty() {
        let refused: usize = did_not_run.values().sum();
        println!("programs that DID NOT RUN {refused}  (nothing in them is counted above)");
        for (why, n) in &did_not_run {
            println!("  {n:>7}  {why}");
        }
    }
