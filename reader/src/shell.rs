...
//!
//! Much of the fleet's file use never passes through the `Write` or `Edit` tools: a
//! `sed -i`, a heredoc or a `cp` changes a file as surely as any of them. Counting
//! only the tools that announce themselves undercounts, and undercounts *unevenly*
//! — an agent that reaches for `sed` loses work an agent that reaches for `Edit`
//! keeps.
//!
...
//! `cargo run --bin shell-report -- <corpus>` is what asks. Restrictive on purpose:
//! a grammar that accepts everything tells you nothing about what it read.
...
///
/// A three-point domain over "did this run", ordered by how much it takes on trust.
/// **A file use is only worth recording if the command that made it actually ran**,
/// and the separator before a command is the only thing in the script that says
/// whether it did.
///
/// [`Reached::and`] is the meet: a command inside a group that may not run is itself
/// uncertain, whatever separator precedes it.
///
...
pub enum Reached {
    ...
    #[serde(rename = "s")]
    OnSuccess,
    /// After `||`, or under some condition this does not model. Runs sometimes, and the
...
pub struct Simple {
    ...
    pub reached: Reached,
    /// The subshells enclosing this command, outermost first — `[]` at the top level,
    /// `[1]` inside the first `( … )`, `[1, 2]` inside a group within it.
    ///
    /// The commands come back as a flat list, which loses the one thing a subshell is
    /// *for*: `(cd android && ./gradlew build)` must not move the script's directory.
    /// Without this, every command after that group resolves against `android/` and
    /// names files that were never touched.
    ///
    /// Ids rather than a depth, because depth alone cannot tell `(cd a && x); (cd b &&
    /// y)` from one group containing both. A brace group gets no id, because in bash it
    /// forks no shell — `{ cd x; }` really does move the caller.
    pub scope: Vec<usize>,
    ...
    ///
    /// Data rather than shell — a commit message, YAML, a SQL script — and never parsed
    /// as shell. It is kept because some of that data is *itself* a program that changes
    /// files, and much of the corpus's heredocs feed `python3 -`. Whether a body is
...
/// them.
///
/// A body is not shell — commit messages, Python, YAML — and parsing prose as shell
/// finds commands nobody ran. It is taken out here rather than in the grammar
/// because a heredoc is the one construct whose terminator is chosen at runtime.
///
/// It is **encoded into the delimiter** rather than discarded, because some of that
/// data is a program in its own right: `python3 - <<'PY'` changes files as surely as
/// `sed -i` does. Encoded into the text rather than set aside in a table, because
/// the corpus's commonest heredoc is `bash -c 'python3 - <<PY … PY'` — the body
/// belongs to the *inner* command, whose script is re-parsed later from a substring
/// of this very text, where a table keyed on this pass would be out of reach.
///
/// Honours the quoted (`<<'EOF'`) and indented (`<<-`) forms, and leaves `<<<`
/// alone: a here-string has no body.
...
/// quoted argument — `bash -c 'python3 - <<PY … PY'` — where the *inner* shell sees
/// a bare `PY` but the text on disk reads `PY'`. So a delimiter followed only by the
/// punctuation that closes the construct around it counts, and that punctuation —
/// never the delimiter itself — is what is kept.
...
///
/// Order is the running order: the commands inside `$( … )`, `<( … )` and a subshell
/// run before the command they belong to.
///
/// `scope` is the chain of subshells the node sits in and `next` hands out the ids;
/// `reached` is the condition from everything enclosing it, refined by the
/// separators between siblings as the walk moves along them.
fn walk(
    ...
    match pair.as_rule() {
        ...
        // `case x in a) … ;; b) … ;; esac`. The arms are alternatives, so at most one of
        // them ran and none is certain — the same [`Reached::Sometimes`] that [`branch`]
        ...
        // called at all.
        //
        // The body is kept rather than dropped, because when the function IS called its
        // commands are the only place those effects appear — a call site is an `Unknown` op
        // that names no files. So it lands in [`Reached::Sometimes`], which is precisely
        ...
        _ => {
            ...
            // `reader/tests/suite/oracle.rs`, one level up.
...
///
/// `if`, `then`, `else` and `fi` are not structure to this grammar: they survive as
/// ordinary words at the front of a command. So the sequence is walked with a stack,
/// one entry per open `if`, and everything inside a branch is
/// [`Reached::Sometimes`].
///
/// The condition itself stays `Always`: `if grep -q x a.txt` really does run it. An
/// `elif` does not — it is reached only when the test before it failed — so it goes
...
///
/// Stops at the first word that is not one, which is the command being guarded. A
...
/// `'$(rm -rf /)'` is text and running it would be inventing a command.
fn expansions(
